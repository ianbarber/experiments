...
def cmd_anchor(args) -> int:
    """Read a JSON request on stdin ({hits, tokens, ignore_case, grep_files, base_dir,
    budget, max_hits}) and print the anchors addendum (empty output = nothing found)."""
    import json
    ...
    if not isinstance(payload, dict):
        return _fail("expected a JSON object on stdin")
    return _run_op("anchor", args, payload, None)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = sub.add_parser("anchor", help="structural facts for grep hits (JSON on stdin; used by harness)")
    _add_common(p)
