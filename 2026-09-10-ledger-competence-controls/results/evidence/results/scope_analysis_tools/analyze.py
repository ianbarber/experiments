#!/usr/bin/env python3
"""Weights-free analysis of the completed amended competence scope; never run models."""
...
_scope_spec = importlib.util.spec_from_file_location('_competence_scope_support', DRAFT / 'scope_support.py')
scope_support = importlib.util.module_from_spec(_scope_spec)
_scope_spec.loader.exec_module(scope_support)
...
class Evidence:
    def __init__(self, root, allow_projection=False, scope_completion=None):
        ...
        self.completion = (self.json('results/COMPETENCE_SCOPE_COMPLETED.json') if scope_completion is None
                           else read_json_value(scope_completion))
        self.scope_terminal_path = 'results/COMPETENCE_SCOPE_COMPLETED.json'
        self.in_memory_finalization = scope_completion is not None
        require(self.completion.get('status') == 'complete', 'Program has not completed')
        self.outcome = self.completion.get('outcome')
        require(self.outcome == 'competence_completed_conditional_cancelled', 'Unsupported competence-scope outcome')
        ...
        scope_support.validate_terminal_evidence(self)
...
def validate_program(evidence, modules, parsed):
    ...
    scope_support.validate_numerical_path(evidence, selection, decision, stop)
    actual_plan = [stage['plan'] for stage in evidence.stages.values()]
    require(len(actual_plan) == 58 and canonical(actual_plan) == canonical(expected),
            'Retained competence stage sequence/settings differ')
    scope_support.validate_model_inventory(evidence)
    return {'base': {order: summaries['base_calibration_' + order] for order in ('decision_first', 'decision_last')},
            ...
            'cohorts': {}, 'stopped_at': stop, 'selected_installations': {},
            'conditional_scope': {'cancelled': True, 'reason': evidence.completion['reason'],
                                  'original_terminal_route': evidence.completion['original_terminal']['route'],
                                  'original_numerical_prerequisites_passed': decision['passed']}}
...
def render_report(result):
    ...
    require(result['repair'] is None, 'Cancelled scope may not report repair contrasts')
    scope = result['development']['conditional_scope']
    lines += ['Conditional induction and repair were intentionally cancelled after design review: ' + result['completion_reason'], '',
              'Original controller terminal route: ' + scope['original_terminal_route'] + '. '
              'The original numerical prerequisite result is retained separately; validation never triggers fallback to a different recipe. '
              'No repair contrast is estimated. This is not a null or equivalence repair result.']
    lines += ['', 'The key-order intervention changes both the requested serialization and supervised output order. Class weighting changes the training objective on identical cases. '
              ...
              'The cancelled branch cannot establish repair transfer or an archived-context effect.', '',
...
def read_json_value(value):
    return json.loads(canonical(value), object_pairs_hook=unique_object, parse_constant=reject_constant)


def verify_competence_scope(root, scope_completion=None, allow_projection=False):
    """Validate all saved cases/stages; candidate completion may exist only in memory."""
    evidence = Evidence(root, allow_projection=allow_projection, scope_completion=scope_completion)
    parsed = {}
    with frozen_modules(evidence.snapshot) as modules:
        data_check = validate_data(evidence, modules)
        for name, stage in evidence.stages.items():
            if stage['contract']['mode'] == 'generate':
                parsed[name], _ = parse_stage(stage, modules['task'], modules['serialization'])
        development = validate_program(evidence, modules, parsed)
        budgets = training_budgets(evidence)
    evidence.recheck()
    return evidence, {'data_checks': data_check, 'development': development,
                      'training_blocks': len(budgets), 'generation_stages': len(parsed),
                      'reparsed_responses': sum(map(len, parsed.values()))}
...
def analyze(root, output, allow_projection=False):
    ...
    result = {'version': 1, 'synthetic_fixture': synthetic,
        'measurement_scope': 'SYNTHETIC FIXTURE — NOT MODEL MEASUREMENTS.' if synthetic else 'Recomputed actual saved responses from the completed competence scope; conditional induction and repair intentionally cancelled.',
        ...
        'interpretation_limits': ['The finite synthetic task and two supplied optimization seeds limit generality.',
                                 ...
                                 'Conditional repair was intentionally cancelled for design adequacy; original numerical gate results are retained separately and no repair contrast is estimated.',
    ...
    completion = {'status': 'complete', 'created_utc': datetime.now(timezone.utc).isoformat(),
        ...
        'root_program_completion_sha256': evidence.hashes['results/COMPETENCE_SCOPE_COMPLETED.json'],
        'root_terminal_path': 'results/COMPETENCE_SCOPE_COMPLETED.json',
        'scope_support_sha256': sha(DRAFT / 'scope_support.py'),
        'original_terminal': evidence.completion['original_terminal'],
